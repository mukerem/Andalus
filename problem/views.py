...
from contest.views import  create_active_contest_session_in_admin, refresh_active_contest_in_admin
...
def sample_test_case(request, zip, problem):
    ...
    for i in zip.namelist():
        ...
        messages.warning(request, 'There is no secret folder in zip file.')
        return
    for i in zip.namelist():
        if i[-1] == '/' and not i.lower() == 'secret/':
            folder.append(i)
    testcase_name = 1
    for i in folder:
        total_in = 0
        ...
        for j in zip.namelist():
            if j[:len(i)] == i:
                if j[-3:] == '.in':
                    ...
                    out_filename = j
        if total_in == 0:
            messages.warning(request, 'There is no .in format file in ' + i +' directory.')
        if total_in > 1:
            messages.warning(request, 'There is multiple .in format file in ' + i +' directory.')
        if total_out == 0:
            messages.warning(request, 'There is no .out format file in ' + i +' directory.')
        if total_out > 1:
            messages.warning(request, 'There is multiple .out format file in ' + i +' directory.')

        if not total_in == 1 or not total_out == 1:
            continue
        ...
        try:
            validate_testcase_in_file_extension(input_file)
        except ValidationError:
            messages.warning(request, 'Unsupported file extension in ' + i +' directory.')
            continue
        
        try:
            validate_testcase_out_file_extension(output_file)
        except ValidationError:
            messages.warning(request, 'Unsupported file extension in ' + i +' directory.')
            continue
...
def handle_zip_file(request, problem_zip):
    ...
    try:
        ...
        try:
            ...
            try:
                problem = Problem(title=title, short_name=short_name, time_limit=time_limit, register_date = timezone.now(), memory_limit=memory_limit, ballon=ballon)  
                problem.save()
                problem.pdf = _pro
                problem.save()
...
def problem(request):
    ...
    if request.method =="POST":
        if 'zip' in request.POST: 
            zip_form = AddProblemZIP(request.POST, request.FILES)
            form = AddProblem()
    ...
    return render(request, 'all_problems.html', {'problem': total_problems, 'form': form, 'zip_form': zip_form})
...
def edit_problem(request, problem_id):
    ...
    return render(request, 'edit_problem.html', 
                 {'form': form, 'problem': total_problems, 'title': problem.title, 'this_problem_id': problem.id})
...
def testcase(request, problem_id):
    ...
    if request.method == "POST":
        ...
        form = AddTestcase()
    return render(request, 'testcase.html', {'form': form, 'title': problem.title, 'test_case': test_case, 'this_problem_id': problem.pk})
...
def delete_testcase(request, testcase_id):
    ...
    return render(request, 'delete_testcase.html', {'testcase': test_case})
